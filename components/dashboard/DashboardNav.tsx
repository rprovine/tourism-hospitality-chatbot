    ...
    <nav className="sticky top-0 z-40 bg-white border-b border-gray-200 w-full">
      <div className="w-full px-6 lg:px-10">
        <div className="flex items-center justify-between h-24">
              ...
              <div className="relative flex-shrink-0">
                {businessLogo ? (
                  <img 
                    src={businessLogo} 
                    alt="Business Logo" 
                    className="h-11 w-11 rounded-xl object-cover shadow-sm ring-2 ring-gray-100 group-hover:ring-cyan-200 transition-all" 
                  />
                ) : (
                  <div className="h-11 w-11 bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-xl flex items-center justify-center shadow-sm ring-2 ring-gray-100 group-hover:ring-cyan-200 transition-all">
                    <Bot className="h-5 w-5 text-cyan-600" />
                  </div>
                )}
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-base font-semibold text-gray-900 leading-tight group-hover:text-cyan-700 transition-colors truncate">
                  {businessName}
                </span>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-xs text-gray-500 capitalize">{businessTier}</span>
                </div>
              </div>
            </Link>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-1 flex-1 justify-center px-12 max-w-5xl">
              {navItems.map((item) => {
                ...
                if (!isAccessible) {
                  return (
                    <div
                      key={item.href}
                      className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg text-gray-400 cursor-not-allowed relative group whitespace-nowrap"
                    >
